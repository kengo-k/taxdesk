...
export interface ExpenseBreakdownByMonthRequest {
  fiscalYear: string
}

export interface ExpenseBreakdownByMonthResponse {
  saimoku_cd: string
  month: string
  value: number
...
export async function calcExpenseBreakdownByMonth(
  ...
  // 借方の集計を取得
  const karikata = await conn.$queryRaw<ExpenseBreakdownByMonthResponse[]>`
    ...
    where j.nendo = ${input.fiscalYear}
      and j.deleted = '0'
      and kari_kbm.kamoku_bunrui_cd = ${KAMOKU_BUNRUI.EXPENSE}
    group by j.karikata_cd,
            substring(j.date, 5, 2)
    order by j.karikata_cd, month`

  // 貸方の集計を取得
  const kasikata = await conn.$queryRaw<ExpenseBreakdownByMonthResponse[]>`
    ...
    where j.nendo = ${input.fiscalYear}
      and j.deleted = '0'
      and kasi_kbm.kamoku_bunrui_cd = ${KAMOKU_BUNRUI.EXPENSE}
  ...
  // 最終的な結果を生成
  const result: ExpenseBreakdownByMonthResponse[] = []
