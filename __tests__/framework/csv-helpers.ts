...
export class CsvHelpers {
   ...
   */
  static async readCsvFile<T = any>(filePath: string): Promise<T[]> {
   ...
   */
  private static getTypeMapFromPrisma(
    prisma: any,
    tableName: string,
    ...
    if (!model) {
      throw new Error(
        `テーブル '${tableName}' に対応するモデルが見つかりません`,
      )
    }
   ...
   */
  static async importCsvToTable<T = any>(
    ...
    // CSVを読み込み
    const records = await this.readCsvFile(csvPath)

    // 型変換を適用
    let convertedRecords = records

    // 指定されたtypeMapがある場合はそれを使用、なければPrismaから自動取得
    const fieldTypeMap = typeMap || this.getTypeMapFromPrisma(prisma, tableName)
      ...
      for (const [key, type] of Object.entries(fieldTypeMap)) {
        if (newRecord[key] !== undefined) {
          switch (type) {
            ...
            case 'boolean':
              newRecord[key] =
                newRecord[key] === 'true' || newRecord[key] === '1'
              break
            case 'date':
