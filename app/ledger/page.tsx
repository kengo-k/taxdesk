...
export default function LedgerPage() {
  ...
  const [account, setAccount] = useState<string>(getAccountIdFromCode(codeParam))
  ...
  // URLパラメータを更新する関数
  const updateUrlParams = (nendo?: string, code?: string, monthValue?: string, page?: number) => {
  ...
  // 取引データの更新関数
  const handleUpdateTransaction = (id: string, field: keyof (typeof transactions)[0], value: string | number) => {
  ...
  const handleDownloadCSV = () => {
    // CSVヘッダー
    const headers = ["日付", "勘定科目コード", "相手科目", "説明", "借方金額", "貸方金額", "摘要", "残高"]
  ...
  const handleDeleteClick = () => {
    if (selectedRows.length > 0) {
      ...
      toast({
        ...
        description: "削除したい取引の行を選択してから削除ボタンをクリックしてください。",
      })
    }
  }
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <p className="text-sm text-gray-500">
                    {fiscalYear}年度 {month === "unset" ? "全期間" : `${month}月`}
                  </p>
                </div>

                <h4 className="text-lg font-medium mb-4">{getSelectedAccountLabel()}</h4>
            ...
            <Card>
              <CardContent className="py-12 text-center">
                <div className="text-gray-500 mb-4">
                  ...
                  <h3 className="text-lg font-medium">データを表示するには検索条件を設定してください</h3>
                </div>
